import { createInvoices } from "@/services/invoicesServices";
import { formSchemaInvoicesCreate } from "../forms/FormsSchem";
...
export const SubmitCreate = async ({
    ...
    try {
        ...
        formSchemaInvoicesCreate.parse(dataBody);

        // If validation passes, proceed with creating the order
        const response = await createInvoices({ dataBody });
